        ...
        async.mapLimit(heroes, 1,
            ...
            function(err, result) {
                ...
                result.forEach(function(hero) {
                    let resultString = JSON.stringify(hero);
...
function fetchInfo(heroUrl, callback) {
    ...
    superagent.get(heroUrl.trim()).end(function(err, res) {
        if (err) {
            ...
            let heroTitle = $('.hero-title').first().text();
            let heroName = $('.hero-name').first().text();
            let heroType = $('.hero-tag').first().text() + " " + $('.hero-tag').last().text();
            let heroesPicUrlArray = new Array();
            ...
            async.mapLimit(heroesPicUrlArray, heroesPicUrlArray.length,
                function(heroesPicUrl, downloadCallback) {
                    // 对每个角色对象的处理逻辑
                    index++;
                    let savePath = 'img/' + heroName + index + ".jpg";
